...
#define __SYSCALL_DEFINEx(x, name, ...) 					\
	...
	asmlinkage long SyS##name(__MAP(x, __SC_LONG, __VA_ARGS__))	\
	{								\
		const char *syscall_basename = __stringify(name);               \
		long ret = 0;													\
		bool skip = false;                                              \
		long skip_ret = 0;                                              \
		unsigned long args_ptr_array[IGLOO_SYSCALL_MAXARGS] = {0};          \
