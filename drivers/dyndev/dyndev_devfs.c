...
static vm_fault_t my_fault_handler(struct vm_fault *vmf) {
    ...
    zap_vma_ptes(vmf->vma, address & PAGE_MASK, PAGE_SIZE);

    // Insert the page into the user space
    ret = vmf_insert_pfn(vmf->vma, address & PAGE_MASK, pfn);
    if (ret) {
        __free_page(page);
        kfree(kernel_buffer);
        return ret;
...
static int dyndev_mmap(struct file *filp, struct vm_area_struct *vma) {
    ...
    vm_flags_set(vma, VM_MIXEDMAP); // Indicate custom page fault handling
...
static char *rw_devnode(const struct device *dev, umode_t *mode) {
    if (mode) {
